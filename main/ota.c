...
static bool resolve_and_validate_host(const char *url,
  ...
  *is_ipv6 = false;
  ...
  if (!private_ip) {
    ESP_LOGE(TAG,
             "Security violation: OTA via HTTP allowed only for private IPs. "
             "Host: %s",
             host);
    return false;
  }
...
#define APPEND_URL_PART(format, ...)                                         \
  ...
  if (u->field_set & (1 << UF_USERINFO)) {
    APPEND_URL_PART("%.*s@", (int)u->field_data[UF_USERINFO].len,
  ...
  if (u->field_set & (1 << UF_PORT)) {
    APPEND_URL_PART(":%.*s", (int)u->field_data[UF_PORT].len,
                    url + u->field_data[UF_PORT].off);
  }

  if (u->field_set & (1 << UF_PATH)) {
    APPEND_URL_PART("%.*s", (int)u->field_data[UF_PATH].len,
                    url + u->field_data[UF_PATH].off);
  }

  if (u->field_set & (1 << UF_QUERY)) {
    APPEND_URL_PART("?%.*s", (int)u->field_data[UF_QUERY].len,
                    url + u->field_data[UF_QUERY].off);
  }

  if (u->field_set & (1 << UF_FRAGMENT)) {
    APPEND_URL_PART("#%.*s", (int)u->field_data[UF_FRAGMENT].len,
...
static bool validate_and_rewrite_url(const char *url, char *out_url,
  ...
  // If check_url_protocol returns true and out_url is filled (HTTPS case), we
  // are done. However, the function returns true for HTTP as well, but doesn't
  // fill out_url. We need to check if it was HTTPS.
...
void run_ota(const char *url) {
  ...
  // We need to initialize BOTH buffers to ensure no "bleed through" of previous
  // content when flipping buffers during the OTA process.
  ...
  display_clear();
  display_text("OTA Update", 2, 10, 0, 0, 255, 1);
  // Don't flip here, we want to draw the progress bar on top of this background
  // in the loop
